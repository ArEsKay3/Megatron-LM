...
class TestDisabledAndEngineScheduling(PrefixCachingTestBase):
    ...
    def test_generation_epoch_invalidates_coordinator_prefix_assignments(self):
        ...
        coordinator._set_generation_epoch(1)
        ...
        coordinator._set_generation_epoch(2)
        assert coordinator._hash_table == {}
...
class TestMambaPrefixCaching(PrefixCachingTestBase):
    ...
    def test_max_intermediate_states_per_step_formula(self):
        ...
        #   token-based:   ceil(max_tokens / block_size)
        ...
        assert ctx.max_requests >= 3
        expected = min(token_based(ctx), request_based(ctx))
        assert expected == token_based(ctx)
        ...
        # Request-limited regime: few requests but a large token budget.
        ctx2 = self._mctx(block_size_tokens=256, max_tokens=2048, max_requests=2)
        expected2 = min(token_based(ctx2), request_based(ctx2))
        assert expected2 == request_based(ctx2)
        assert expected2 < token_based(ctx2)
    ...
    def _build_inference_context(
        ...
        return DynamicInferenceContext(
            ...
            inference_config=InferenceConfig(
                ...
                async_sched_mode=getattr(test_config, "async_sched_mode", AsyncScheduleMode.LEGACY),
            ),
        )
