...
def prepare_trajectories(
    ...
    if tokenizer.library == "huggingface":
        _tokenizer : HuggingFaceTokenizer = tokenizer._tokenizer
        if not tokenizer.pad_id:
            for pad_token in DEFAULT_PAD_TOKENS:
                if pad_token in _tokenizer.tokenizer.get_vocab():
                    log_single_rank(
                        logger, logging.INFO, f"Updating tokenizer pad token to {pad_token}"
                    )
                    _tokenizer.pad_token = pad_token
                    break
            else:
