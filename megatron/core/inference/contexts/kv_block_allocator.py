...
class KVBlockAllocator:
    ...
    def get_allocatable_block_count(self, reserved_evictable: int = 0) -> int:
        """Get the number of blocks available for allocation.

        Includes both free pool blocks and registered, evictable cached blocks.

        Args:
            reserved_evictable (int): Number of currently-evictable cached
    ...
    def is_memory_available(self, num_blocks: int, reserved_evictable: int = 0) -> bool:
        ...
        if self.total_avail >= num_blocks:
            return True
        return self.get_allocatable_block_count(reserved_evictable) >= num_blocks
